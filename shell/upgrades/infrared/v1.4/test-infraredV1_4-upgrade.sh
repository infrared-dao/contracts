...
DEPLOY_OUTPUT=$(forge script script/upgrades/infrared/v1.4/DeployInfraredV1_4.s.sol:DeployInfraredV1_4  --broadcast  --rpc-url $RPC_URL  --private-key $PRIVATE_KEY -vvvv 2>&1)
INFRARED_V4_IMPLEMENTATION=$(echo "$DEPLOY_OUTPUT" | grep -oP '(?<=new InfraredV1_4@)0x[a-fA-F0-9]{40}' | head -n1)

echo "Extracted Address: $INFRARED_V4_IMPLEMENTATION"
if [[ -z "$INFRARED_V4_IMPLEMENTATION" ]]; then
    echo "Error: Failed to extract deployment address!"
    kill "$ANVIL_PID"
    exit 1
fi
INFRARED_V4_IMPLEMENTATION=$(echo "$INFRARED_V4_IMPLEMENTATION" | tr -d '\n' | tr -d '\r')

echo "InfraredV4 deployed at: $INFRARED_V4_IMPLEMENTATION"
