...
contract UpgradeInfraredTestnetV1_7 is Script {
    ...
    function deployCollector(
        ...
        if (!collector.hasRole(collector.GOVERNANCE_ROLE(), msg.sender)) {
    ...
    function upgradeInfrared(
        ...
        vm.startBroadcast();
        InfraredV1_7(payable(_infraredProxy)).upgradeToAndCall(
            newInfraredImp, initializerData
        );
        vm.stopBroadcast();
    }
